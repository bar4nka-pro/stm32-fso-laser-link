                         ...
                         uint8_t *out, size_t out_cap)
{
    ...
    if (out_cap < (size_t)len + OVERHEAD)  return 0;
...
result_t feed_byte(parser_t *p, uint8_t byte)
{
    switch (p->state) {
        ...
        case ST_SYNC1:
            if (byte == SYNC1) p->state = ST_SYNC2;
            return IDLE;

        case ST_SYNC2:
            if (byte == SYNC2)      p->state = ST_LENGTH;
            else if (byte != SYNC1) p->state = ST_SYNC1;
        ...
        case ST_DATA:
            ...
            p->checksum ^= byte;
            if (p->index >= p->length) p->state = ST_CHECKSUM;
            return IDLE;
